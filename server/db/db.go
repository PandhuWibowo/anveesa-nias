...
func migrate() error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS analytics_dashboards (
			...
			presets_json TEXT DEFAULT '[]',
			default_preset TEXT DEFAULT '',
			created_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
			updated_at  DATETIME DEFAULT CURRENT_TIMESTAMP
		)`,
		`ALTER TABLE analytics_dashboards ADD COLUMN visibility TEXT NOT NULL DEFAULT 'private'`,
		`ALTER TABLE analytics_dashboards ADD COLUMN share_token TEXT DEFAULT ''`,
		`ALTER TABLE analytics_dashboards ADD COLUMN presets_json TEXT DEFAULT '[]'`,
		`ALTER TABLE analytics_dashboards ADD COLUMN default_preset TEXT DEFAULT ''`,
		`CREATE INDEX IF NOT EXISTS idx_analytics_dashboards_visibility_owner ON analytics_dashboards(visibility, user_id)`,
		`CREATE INDEX IF NOT EXISTS idx_analytics_dashboards_share_token ON analytics_dashboards(share_token)`,
		`CREATE TABLE IF NOT EXISTS analytics_dashboard_access (
			id             INTEGER PRIMARY KEY AUTOINCREMENT,
		...
		`CREATE INDEX IF NOT EXISTS idx_analytics_dashboard_access_dash ON analytics_dashboard_access(dashboard_id, access_level)`,
		`CREATE INDEX IF NOT EXISTS idx_analytics_dashboard_access_user ON analytics_dashboard_access(user_id, dashboard_id)`,
		`CREATE TABLE IF NOT EXISTS analytics_dashboard_blocks (
			id             INTEGER PRIMARY KEY AUTOINCREMENT,
			dashboard_id   INTEGER NOT NULL REFERENCES analytics_dashboards(id) ON DELETE CASCADE,
		...
		`ALTER TABLE analytics_dashboard_blocks ADD COLUMN column_span INTEGER DEFAULT 1`,
		`ALTER TABLE analytics_dashboard_blocks ADD COLUMN row_span INTEGER DEFAULT 2`,
		`ALTER TABLE analytics_dashboard_blocks ADD COLUMN params_json TEXT DEFAULT '[]'`,
		`CREATE INDEX IF NOT EXISTS idx_analytics_dashboards_user_time ON analytics_dashboards(user_id, updated_at DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_analytics_dashboard_blocks_dash_order ON analytics_dashboard_blocks(dashboard_id, sort_order ASC, id ASC)`,
		`ALTER TABLE schedules ADD COLUMN dashboard_id INTEGER DEFAULT 0`,
