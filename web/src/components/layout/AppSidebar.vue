...
function selectConn(conn: Connection) {
  ...
  const stayViews = ['schema', 'data', 'er', 'dashboard', 'query', 'redis', 'memcache', 'mongodb', 'search', 'search-policies', 'laravel-queue', 'kafka']
  ...
  else if (conn.driver !== 'redis' && conn.driver !== 'memcache' && conn.driver !== 'kafka' && conn.driver !== 'mongodb' && !isSearchDriver(conn.driver) && ((current === 'redis' || current === 'memcache' || current === 'kafka' || current === 'mongodb' || current === 'search') || !stayViews.includes(current))) router.push({ name: 'query' })
}
