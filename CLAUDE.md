...
- **Cron / Scheduler (servers)** — `handlers/cron.go` is a **native crontab manager**: it reads and edits each server's own crontab live over SSH rather than storing jobs in Anveesa (so schedulers already on the VMs show up and are editable). No cron DB tables. It **reuses the shared SSH Hosts (`docker_hosts`)** as targets — `GET /api/cron/hosts` is a cron-permissioned pick-list over `docker_hosts` (so cron users don't need Docker perms), and all SSH work loads the host via `loadDockerHost` + `sshClientForHost`. Endpoints: `GET /api/cron/hosts/{id}/crontab` runs `crontab -l` (plus a best-effort read-only cat of `/etc/crontab` + `/etc/cron.d/*` as `system_crontab`); `PUT .../crontab` writes the connecting user's crontab back via `crontab -` (stdin), surfacing crontab's own validation errors; `POST .../run` executes one command immediately. `runSSHCommand` captures stdout/stderr + exit code with a timeout that kills the session (non-zero exit is an outcome, not a transport error). Crontab parsing/serialization (into editable entries + preserved comment/env lines, with disable = comment-out) lives in the frontend `CronView.vue`, which is a host list + per-host table/raw crontab editor with run-now output. Permissions: `cron.view`, `cron.manage`, `cron.exec` (run-now, high-risk).
